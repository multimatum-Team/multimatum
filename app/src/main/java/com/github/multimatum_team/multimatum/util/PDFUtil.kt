...
object PDFUtil {
     ...
     * Upload a pdf file
     */
    fun uploadPdfToFirebase(data: Uri, storageRef:StorageReference, context:Context, uniqueId:Int, callback: (String) -> Unit) {
        if (data != Uri.EMPTY) {
            val ref =
                storageRef.child(FirebaseAuth.getInstance().uid + "/upload" + uniqueId.toString() + getFileNameFromUri(data))
            ref.putFile(data).addOnSuccessListener {
                callback(ref.path)
            }.addOnFailureListener {
                val failureDialog = AlertDialog.Builder(context).setTitle("pdf upload failed").show()
                callback("")
            }
        } else {
            callback("")
